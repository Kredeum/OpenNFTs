...
contract OpenClonerEx is OpenCloner {
    function clone(address template_) public override(OpenCloner) returns (address clone_) {
        ...
        options[0] = true;
        IOpenCloneable(clone_).initialize(abi.encode("Cloned by OpenClonerEx", "TSTEX", msg.sender, options));
    }

    function supportsInterface(bytes4 interfaceId) public view override(OpenCloner) returns (bool) {
